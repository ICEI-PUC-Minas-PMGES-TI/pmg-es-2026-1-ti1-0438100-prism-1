...
fetch('postos.json')
  ...
  .catch(() => {
    document.getElementById('postoList').innerHTML =
      '<div style="padding:16px;color:#d32f2f;font-size:.85rem;">⚠️ Não foi possível carregar postos.json.</div>';
  });
...
function inicializarDadosEMapa() {
  ...
  // 1. Simulação do Mapa de Calor (Círculos concêntricos transparentes com gradiente)
  const heatLayer = L.layerGroup();
  postos.forEach(p => {
    const norm = (p.atendimentos - minAtt) / (maxAtt - minAtt);
    const radius = 600 + norm * 1400;
    
    let heatColor = '#ff2200'; // Vermelho (Alta Demanda)
  ...
  // 2. Criação dos marcadores clássicos de localização
  postos.forEach((p, i) => {
    let col = '#1a3fa4';
...
function encontrarMaisProximo() {
  ...
  document.getElementById('nearbyResult').innerHTML = '<div style="padding:4px 0">📍 Obtendo localização…</div>';
  ...
  navigator.geolocation.getCurrentPosition(posicao => {
    ...
    postos.forEach((p, i) => {
      const d = calcularDistancia(minhaLat, minhaLon, p.lat, p.lon);
      if (d < menorDistancia) { 
        menorDistancia = d; 
        postoMaisProximo = p; 
        melhorIdx = i; 
    ...
    document.getElementById('nearbyResult').innerHTML = `
      ...
      <div style="font-size:.78rem;color:#5a6380;margin:4px 0 8px">${postoMaisProximo.endereco}</div>
      <span class="dist">📍 ${menorDistancia.toFixed(2)} km de distância</span>
    `;

    map.setView([postoMaisProximo.lat, postoMaisProximo.lon], 15, { animate: true });
    markers[melhorIdx].openPopup();
    focarItemNaLista(melhorIdx);
  }, () => {
    document.getElementById('nearbyResult').innerHTML = '<div style="color:#d32f2f">Não foi possível obter sua localização.</div>';
  });
}
